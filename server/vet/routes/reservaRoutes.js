...
import { reservaSchema, paginationSchema } from "../validators/schemas.js"

export default function reservaRoutes(getController) {
  const router = express.Router()

  // Solo clientes pueden crear reservas
  router.post("/petcare/reservar", authMiddleware, authorizationMiddleware('cliente'), validate(reservaSchema), (req, res, next) => {
  ...
  router.get("/petcare/reservas", authMiddleware, authorizationMiddleware('cliente', 'veterinaria', 'paseador', 'cuidador'), validate(paginationSchema, 'query'), (req,res,next) => {
